...
# If a user makes a choice that they want snort to run on bootup, but do not configure the snort interface to be up on system startup
	case $startup_choice in
		...
		if [ $boot_iface = "1" ]; then
			...
			echo "/usr/local/bin/barnyard2 -c /usr/local/snort/etc/barnyard2.conf -S /usr/local/snort/etc/sig-msg.map -G /usr/local/snort/etc/sig-msg.map -d /var/log/snort -f snort.u2 -w /var/log/snort/barnyard2.waldo -D" >> /root/rc.local.tmp

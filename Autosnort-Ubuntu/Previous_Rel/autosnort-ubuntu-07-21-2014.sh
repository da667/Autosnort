...
print_status "Installing base packages: ethtool build-essential libpcap0.8-dev libpcre3-dev bison flex autoconf libtool libmysqlclient-dev libnetfilter-queue-dev libnetfilter-queue1 libnfnetlink-dev libnfnetlink0.."

declare -a packages=( ethtool build-essential libpcap0.8-dev libpcre3-dev bison flex autoconf libtool libmysqlclient-dev libnetfilter-queue-dev libnetfilter-queue1 libnfnetlink-dev libnfnetlink0 );
...
" ui_inst
	case $ui_inst in
		...
		print_status "Backing up default ssl.conf, default virtual host files, and replacing ssl.conf.."
	
		mv /etc/apache2/sites-available/*default* /etc/apache2
		mv /etc/apache2/mods-available/ssl.conf /etc/apache2/
...
#We run ethtool to disable generic and large recieve offloading. This can affect packet checksums, and if the packet decoder is configured to discard what it thinks to be bad or invalid checksums, this can result in missed alerts.
...
#The choice above determines whether or not we'll be adding an entry to /etc/sysconfig/network-scripts  for the snort interface and adding the rc.local hack to bring snort's sniffing interface up at boot. We give the user the choice of not doing this, in the case that they may not have two dedicated network interfaces available.

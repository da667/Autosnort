...
#we ask the user for a password for snort earlier. here's where we build the mysql database and give rights to the snort user to manage the database. all of these are now encapsulated in while true loops for better fault tolerance.
...
while true; do
	...
	case $ui_choice in
		...
		if [ $? != 0 ]; then
			echo "It looks like the installation did not go as $Paccording to plan."
			echo "Verify you have network connectiviy and try again"
			continue
		else
			echo "Installation successful!"

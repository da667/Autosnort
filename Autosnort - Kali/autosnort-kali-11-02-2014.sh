...
#These lines establish where autosnort was executed. The config file _should_ be in this directory. the script exits if the config isn't in the same directory as the full-autosnort-kali shell script.
...
if [[ $release == "1.0."* ]]; then
	...
	print_notification "You continue at your own risk!"
fi
...
print_status "Installing base packages: libpcap0.8-dev bison flex libtool libmysqlclient-dev libnetfilter-queue-dev libnetfilter-queue1 libnfnetlink-dev libnfnetlink0 perl libarchive-tar-perl libcrypt-ssleay-perl libwww-perl"

declare -a packages=( libpcap0.8-dev bison flex libtool libmysqlclient-dev libnetfilter-queue-dev libnetfilter-queue1 libnfnetlink-dev libnfnetlink0 perl libarchive-tar-perl libcrypt-ssleay-perl libwww-perl );
install_packages ${packages[@]}

########################################

#The user chooses whether or not they want to install software to support a web-based IDS console.
#If they choose to install the pre-reqs to support a web-based IDS console, the script installs the necessary software packages, configures apache and mysql to run on startup, runs commands equivalent to the mysql_secure_installation script (automatically), generates a private key and self-signed SSL cert, and lays down an HTTP to HTTPS mod_rewrite virtual host to enforce web console encryption regardless of the web interface installed.

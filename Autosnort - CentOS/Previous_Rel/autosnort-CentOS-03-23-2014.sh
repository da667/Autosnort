...
########################################
#This is a postprocessing function that should get ran after pulled pork is ran in the rules processing portion of the script. The code is identical in all cases, so it made sense to made a function for code re-use.
#This block of code ensures the so_rules.rules file exists, and touches it into existence if it doesn't, notifies the user where pulledpork is installed, moves the other snort configuration files (e.g. classification.config, etc.), and adds relevant lines to snort.conf to point to the proper rule files.
#Change with rule tarballs around snort 2.9.6.0 or so: gen-msg.map is no longer distrbuted with rule tarballs.
#Change to the script to copy it from the source tarball etc directory.
...
if [[ $release == "6."* ]]; then
    ...
    print_notification "This is not CentOS. Be aware this script has NOT been tested on other platforms (Including RHEL, Fedora and/or SuSE)."
...
if [ $? -eq 0 ]; then
	...
	wget https://dl.fedoraproject.org/pub/epel/$epelrel/$arch -O index.html &>> $logfile
	if [ $? -ne 0 ]; then
		print_error "failed to reach dl.fedoraproject.org."
		exit 1
	fi
	epel_package=`grep epel-release index.html | cut -d\" -f2`
	rm -rf index.html
...
" ui_inst
	...
	declare -a packages=( httpd mysql mysql-bench mysql-server );
	install_packages ${packages[@]}
	...
	if [ $? -ne 0 ]; then
		...
		print_good "Secure installation script completed. Mysql and HTTPD successfully installed."
	fi
	break
	;;
	2)
        ...
        case $rule_install in
            ...
            test -e $rule_dir/$rule_filename
                if [ $? = 0 ]; then
					...
					if [ $? != 0 ]; then
						print_notification "The filename you have supplied is not a tarball or could not be read by tar. Please try again."
						continue
					else
...
print_notification "Be aware that RHEL derivatives ship with iptables enabled by default. If you elected to install a web interface, use the command system-configure-firewall-tui to allow access to WWW inbound on this sytem to access your web interface!"

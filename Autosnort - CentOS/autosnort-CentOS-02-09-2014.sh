...
{
if [ ! -f /usr/local/snort/so_rules/so_rules.rules ]; then
	print_notification "so_rules.rules doesn't exist. Creating.."
	touch /usr/local/snort/so_rules/so_rules.rules
fi
...
if [ $? -eq 0 ]; then
	...
	wget https://dl.fedoraproject.org/pub/epel/$epelrel/$arch &>> $logfile
	if [ $? -ne 0 ]; then

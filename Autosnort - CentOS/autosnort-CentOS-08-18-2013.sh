...
if [ $release = "6" ]; then
    ...
    print_notification "This is not CentOS. Be aware this script has NOT been tested on other platforms (Including RHEL and/or SuSE)"
        while true; do
...
if [ $? -ne 0 ]; then
    print_error "YUM update failed. Please check /var/log/autosnort_install.log for details."
	exit 1	
else
...
" ui_inst
	...
	print_notification "We're now going to run /usr/bin/mysql_secure_installation."
	print_notification "This is in order to secure the mysql install and give you the opportunity to set the root mysql user password."
	print_notification "Please be sure to record/remember the root mysql password, you will need it later!"
...
while true; do
	...
	case $ui_choice in
		1)
		print_status "You have chosen to install Snortreport."

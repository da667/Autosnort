...
mkdir -p AS_offline_ubuntu32/apt_pkgs/archives/partial

apt-get install -y -d -o dir::cache=./AS_offline_ubuntu32/apt_pkgs ethtool nmap nbtscan apache2 php5 php5-mysql php5-gd libpcap0.8-dev libpcre3-dev g++ bison flex libpcap-ruby make autoconf libtool mysql-server libmysqlclient-dev
...
mkdir AS_offline_ubuntu32/sources
cd AS_offline_ubuntu32/sources
...
cd ../..
mv dpkgorder.txt AS_offline_ubuntu32/apt_pkgs/archives/
tar -cvzf AS_offline_ubuntu32.tar.gz

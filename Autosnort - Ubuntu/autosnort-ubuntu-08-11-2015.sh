...
daqtar=`grep daq-[0-9] /tmp/snort | grep .tar.gz | head -1 | cut -d"/" -f6 | cut -d"<" -f1`
...
wget https://www.snort.org/documents/snort-$choice1conf-conf -O $snort_basedir/etc/snort.conf --no-check-certificate &>> $logfile
...
if [ $? != 0 ];then
	...
	wget https://www.snort.org/documents/snort-$choice2conf-conf -O $snort_basedir/etc/snort.conf --no-check-certificate &>> $logfile

...
print_status "OS Version Check."
    ...
    if [ $OS != "Debian" ]; then
        print_notification "/etc/issue.net Did not return Debian as the OS. This script has not been tested on other platforms, and is designed solely for Debian systems."
		print_notification "If you've modified /etc/issue.net, and you're absolutely certain this is a Debian System, continue. Otherwise be warned: This script has NOT been tested on Non-Debian systems."
            while true; do
...
# Using lsb_release -c to make sure we're using the right packages for the right Debian release.

debrelease=`lsb_release -c | awk '{print $2}'`

print_status "Adding deb and deb-src via http://packages.dotdeb.org to apt sources."
...
print_good "Successfully added packages.dotdeb.org to /etc/apt/sources.list"
print_status "Adding packages.dotdeb.org gpg key."

cd /usr/src
wget http://www.dotdeb.org/dotdeb.gpg &>> $logfile && cat dotdeb.gpg | apt-key add - &>> $logfile
if [ $? -ne 0 ]; then
    print_error "Failed to install key for packages.dotdeb.org. check /var/log/autosnort_install.log for details."
	exit 1	
else
    print_good "Key installed."
fi
...
print_status "Installing base packages: ethtool build-essential zlib1g-dev libpcap-dev libpcre3-dev bison flex libpcap-ruby autoconf libtool libmysqlclient-dev"

declare -a packages=( ethtool build-essential zlib1g-dev libpcap-dev libpcre3-dev bison flex libpcap-ruby autoconf libtool libmysqlclient-dev );
...
print_status "configuring snort (options --prefix=/usr/local/snort and --enable-sourcefire), making and installing. This will take a moment or two."2
...
Select 2 to download rules for snort $choice2 (Select this if it has been less than 30 days since snort $choice1 has been released and you do NOT have a VRT rule subscription oinkcode)
...
while true; do
	...
	case $ui_choice in
		1)
		print_status "Installing Snortreport."
		...
		if [ $? != 0 ]; then
			...
			print_notification "Or you can use the web interface to start the worker process, and select the force cache update option on the main page. that should work as well."
			print_notification "Copy this down, because I also advise rebooting this system before putting it into production."
			break
		fi
		;;
		6)
